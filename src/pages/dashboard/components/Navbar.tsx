...
export default function NavbarDashboard() {
    ...
    return (
        <>
            ...
            <nav className="hidden max-sm:flex h-max justify-end items-center gap-2 w-full">
                <DropdownMenuDemo />
                <button
                    className="z-10 transition-all duration-700 lg:hidden"
                    onClick={() => { HandleOpenMenu() }}
                >
                    {
                        isOpen ?
                            <MdMenu className="text-gray-200 w-6  h-6" />
                            :
                            <MdMenuOpen className="text-gray-200 w-6  h-6" />
                    }
                </button>
            </nav>
        </>
